...
launch() {
  ...
  local cmd
  cmd="podman run -d --name kotb-ralph-m10-${tribe} --userns=keep-id \
...
BODY
  ...
  # Extract just the PR number from the URL for reviewer request
  local pr_n
  pr_n=$(echo "$pr_num" | grep -oE '[0-9]+$' || true)

  if [ -n "$pr_num" ]; then
    # Request Copilot review via gh cli (preferred over raw API)
